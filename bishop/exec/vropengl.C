...
void pack( Frame& f, const Image& v );
...
int main(int argc, char** argv)
{
    ...
    viewer.push_back( volumeImage.Width(), volumeImage.Height(), GL_RGB ); 
    pack( viewer.GetFrame(viewer.size()-1), volumeImage );
    viewer.AddTextureToList();
...
void pack( Frame& f, const Image& v )
{
   ...
   f.label = "rendered";
   f.data_type = GL_RGB;
   f.aspect = (float)v.Width() / (float) v.Height();
   f.gamma = 1.0;
   f.contrast = 1.0;
   f.scale_low = 10000000.0;
   f.scale_high = -10000000.0;
   f.data.resize( f.width*f.height*3 );
   long index = 0;
   for( int y=0;y<f.height;y++)
   {
      int j = (int) (  (float)y * (float) (v.Height()-1) / (float) (f.height-1) );
      for( int x=0;x<f.width;x++ )
      {
         int i = (int) (  (float)x * (float) (v.Width()-1) / (float) (f.width-1) );
         for( int c=0;c<3;c++ )
	 {
	    float a = v.value(i,j,c);
	    f.data[index++] = a;
	    f.scale_low = ( f.scale_low > a ) ? a : f.scale_low;
	    f.scale_high = ( f.scale_high < a ) ? a : f.scale_high;
	 }
      }
   }
}
