   ...
   ~ProgressMeter()
    {
       ...
       cout << "\r" << title << ":   Total time to finish: " << formattedTime(currentUsedTime) << "                                                                                \n\n" << flush;
    ...
    void update()
    {
       ...
       if( currentPercent > oldPercent || ( currentUsedTime > 0 && initial) )
       {
          ...
          if( totalnumber > 1 ) {cout << "\r" << title << ":   " << formattedPercent(currentPercent) << "   U: " << formattedTime(currentUsedTime) << "   L: " << formattedTime(estimatedTimeRemaining) << "   T: " << formattedTime(estimatedTotalTime) << "                      " << flush; }
       }
    }
