...
def CoiledSpring( nbTurns, nbVerts, springRadius, springGap, thick, capthick, ybase ):
	...
	for j in range(0,nbTurns):
		for i in range(0,nbVerts):
			...
			if i == 0 and j == 0:
				P0 = noiseparms.P
			noiseparms.radius = thick
			noiseparms.capradius = capthick
...
for j in range(0,nbTurns):
	for i in range(0,nbVerts):
		...
		noiseparms.capradius = capthickness
		noiseparms.translate = P0
		noiseparms.gamma = 0.3
		noiseparms.amplitude = 0.2
		noiseparms.frequency = 2.33428293
...
for frame in range(299,nbFrames):
	...
	stamp( densityGrid, volumeObject, 10 )
