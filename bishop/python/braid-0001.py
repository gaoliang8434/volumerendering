...
llc = Vector(-0.5,-2,-0.5)
urc = Vector(0.5,2,0.5)
cellSize = 0.025
...
# left tube
gl = TriangleGeometry()
gl.setScaling( allparms["-objscale"] )
fillresult = p.Fill(gl)
displaceGeometry( gl, constant(Vector(-0.1,0,0)) ) 

# right tube
gr = TriangleGeometry()
gr.setScaling( allparms["-objscale"] )
fillresult = p.Fill(gr)
displaceGeometry( gr, constant(Vector(0.1,0,0)) ) 

g.merge(gl)
g.merge(gr) 
        
displaceGeometry( g, YY )
outfilename = "./braid-0001." + formattedFrame( f ) + ".obj"
