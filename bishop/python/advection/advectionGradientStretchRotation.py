...
for nbIterations in range(1,maxIterations+1):
	...
	for i in range(0,nbIterations):
		...
		maps[0] = PutInGrid(  gradientStretchCM( velocityField, T, 20 ) - identity(), gridbox, Vector(0.0,0.0,0.0) ) + identity()
	# Displace and Render
	...
	for i in range(0, len(maps) ):
		density = density + translate( warp( baseVolume, maps[i] ), hdisplacement * float( i - int(len(maps)/2.0) ) ) 
	allparms["-case"] = "GriddedGSMaps"
	allparms["-turntableframe"] = nbIterations
	StandardRender( density, allparms )
