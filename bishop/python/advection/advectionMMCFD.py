...
velocityField = cross( grad(noisefield1), grad(noisefield2) ) * constant( 5.0 )
#velocityField = constant(Vector(0,0,0))
...
T = 1.0/24.0
gravity = constant (Vector( 0, -10*0.0, 0 ))
referenceDensity = constant(1.0)
maxIterations = int(allparms["-nbadvectioniterations"])
for frame in range(1,100+1):
	print "\n\nFrame " + str(frame) + "\n\n"
	stepmap = PutInGrid(  advectModifiedMacCormack( identity(), velocityField, T ) - identity(), gridbox, Vector(0.0,0.0,0.0) ) + identity()
	cfdmap = PutInGrid( warp(cfdmap, stepmap ) - identity(), gridbox, Vector(0,0,0) ) + identity()
	density = warp(baseVolume, cfdmap )
	velocityField = warp(velocityField, stepmap ) -  constant(T) * density * gravity / referenceDensity
	velocityField = FFTDivFree( gridbox, velocityField )
