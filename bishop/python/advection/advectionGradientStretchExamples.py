...
nfold = 4
T = float(allparms["-advectiontime"]) / math.pow( 2.0, nfold )
dL = 0.02
gridbox = makeGridBox( Vector(-L,-L,-L), Vector(L,L,L), Vector( dL, dL, dL ) )
stepmap = gradientStretchCM( velocityField, T, 1 )
for ii in range(0,nfold):
	stepmap = warp( stepmap, stepmap )
advmap = PutInGrid(  stepmap - identity(), gridbox, Vector(0.0,0.0,0.0) ) + identity()
