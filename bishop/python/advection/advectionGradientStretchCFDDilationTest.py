...
T = baseT
stepmap = PutInGrid(  gradientStretchCM( velocityField, T, 11 ) - identity(), gridbox, Vector(0.0,0.0,0.0) ) + identity()
...
allparms["-case"] = "DilationTestB"
velocityField = cross( grad(noisefield1), grad(noisefield2) )*constant(dilationScale)
velocityField = FFTDivFree( gridbox, velocityField )

cfdmap = identity()

T = baseT/dilationScale
stepmap = PutInGrid(  gradientStretchCM( velocityField, T, 10 ) - identity(), gridbox, Vector(0.0,0.0,0.0) ) + identity()
cfdmap = PutInGrid( warp(cfdmap, stepmap ) - identity(), gridbox, Vector(0,0,0) ) + identity()
density = warp(baseVolume, cfdmap )
StandardRender( density, allparms )
