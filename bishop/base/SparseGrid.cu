...
// SGrid<T> destructor with specializations
template <>
lux::SGrid<float>::~SGrid()
{
   deallocate();
...
// SGrid<T> initCUDA with template specializations
template <>
void lux::SGrid<float>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = new gilligan::DataObject<float>(data_size);

   for(int i = 0; i < totalSize; i++)
      (**cu_mapOfMap)[i] = mapOfMap[i];
   cu_mapOfMap->updateDevice();
   ...
   for(int i = 0; i < data_size; i++)
      (**cu_data)[i] = data[i];
   cu_data->updateDevice();
}

template <>
void lux::SGrid<Vector>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = new gilligan::DataObject<Vector>(data_size);
   ...
   for(int i = 0; i < data_size; ++i)
   {
      (**cu_data)[i].x = data[i][0];
      (**cu_data)[i].y = data[i][1];
      (**cu_data)[i].z = data[i][2];
   }
   cu_data->updateDevice();
}

template <>
void lux::SGrid<Color>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = new gilligan::DataObject<Color>(data_size);
   ...
   for(int i = 0; i < data_size; ++i)
   {
      ...
      (**cu_data)[i].y = data[i][1];
      (**cu_data)[i].z = data[i][2];
      (**cu_data)[i].w = data[i][3];
   }
   cu_data->updateDevice();
}

template <>
void lux::SGrid<Matrix>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = nullptr;
...
void lux::initCUDA( ScalarGrid& grid ) { grid->initCUDA(); }
void lux::initCUDA( VectorGrid& grid ) { grid->initCUDA(); }
void lux::initCUDA( ColorGrid& grid ) { grid->initCUDA(); }
void lux::initCUDA( MatrixGrid& grid ) { grid->initCUDA(); }

/* SparseFrustumGrid CUDA */
template <>
lux::FSGrid<float>::~FSGrid()
{
   deallocate();
...
// FSGrid<T> initCUDA with template specializations
template <>
void lux::FSGrid<float>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = new gilligan::DataObject<float>(data_size);
...
template <>
void lux::FSGrid<Vector>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = new gilligan::DataObject<Vector>(data_size);
...
template <>
void lux::FSGrid<Color>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = new gilligan::DataObject<Color>(data_size);
...
template <>
void lux::FSGrid<Matrix>::initCUDA()
{
   cu_mapOfMap = new gilligan::DataObject<long>(totalSize);
   cu_map = new gilligan::DataObject<long>(map_size);
   cu_data = nullptr;
...
void lux::initCUDA( ScalarFrustumGrid& grid ) { grid->initCUDA(); }
void lux::initCUDA( VectorFrustumGrid& grid ) { grid->initCUDA(); }
void lux::initCUDA( ColorFrustumGrid& grid ) { grid->initCUDA(); }
void lux::initCUDA( MatrixFrustumGrid& grid ) { grid->initCUDA(); }
