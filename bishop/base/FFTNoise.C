...
void FFTNoise::setParameters( const Noise_t& n )
{
   ...
   for( int k=0;k<n.fftNbGridPoints;k++ )
   {
      ...
      for( int j=0;j<n.fftNbGridPoints;j++ )
      {
	    ...
	    float ksq = kx*kx + ky*ky + kz*kz;
            ...
            float amp = 0.0;
	    if( ksq > 0 ) { amp = std::exp( -ksq*n.fftLowCutoff*n.fftLowCutoff ) * std::exp( - 1.0/(ksq*n.fftHighCutoff*n.fftHighCutoff) ) / pow( (double)ksq, (double)n.fftPower*0.5 ); }
   ...
   grid.init( n.fftNbGridPoints, n.fftNbGridPoints, n.fftNbGridPoints, n.fftLength, n.fftLength, n.fftLength, Vector(0,0,0) );
   grid.setPeriodic();
   index = 0;
   mean = 0;
   double stddev = 0;
