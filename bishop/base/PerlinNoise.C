...
/* Macro B in original code modified to PERLINB */
...
void PerlinNoise::init(void)
{
	...
	for (i = 0 ; i < PERLINB ; i++) {
		...
		g1[i] = (float)((random() % (PERLINB + PERLINB)) - PERLINB) / PERLINB;

		for (j = 0 ; j < 2 ; j++)
			g2[i][j] = (float)((random() % (PERLINB + PERLINB)) - PERLINB) / PERLINB;
		normalize2(g2[i]);

		for (j = 0 ; j < 3 ; j++)
			g3[i][j] = (float)((random() % (PERLINB + PERLINB)) - PERLINB) / PERLINB;
		normalize3(g3[i]);
	}

	while (--i) {
		k = p[i];
		p[i] = p[j = random() % PERLINB];
		p[j] = k;
	}
	...
	for (i = 0 ; i < PERLINB + 2 ; i++) {
		...
		g1[PERLINB + i] = g1[i];
		for (j = 0 ; j < 2 ; j++)
			g2[PERLINB + i][j] = g2[i][j];
		for (j = 0 ; j < 3 ; j++)
			g3[PERLINB + i][j] = g3[i][j];
	}
}
